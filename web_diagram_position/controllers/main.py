...
class MyDiagramView(DiagramView):
    ...
    def get_diagram_info(self, req, id, model, node, connector,
        ...
        # Here we have had a problem with searching field by the model name,
        # which caused exception because there are two fields with same model
        ...
        if xpos and ypos:
            ...
            nodes = dict()
            for key, value in ret['nodes'].iteritems():
                nodes[int(key)] = value
            ret['nodes'] = nodes
    ...
    def update_diagram(self, req, id, node, xpos, ypos, x, y):
        http.request.env[node].browse(id).write({xpos: x, ypos: y})
        return http.request.env[node].browse(id)['name']

    @odoo.http.route('/web_diagram/diagram/get_node_info', type='json', auth='user')
    def get_node_info(self, req, id, model, bgcolor, shape, xpos, ypos, **kw):
        node = http.request.env[model].browse(id)
        ...
        n['x'] = node[xpos]
        n['y'] = node[ypos]
        n['color'] = self.eval_node_color(bgcolor, node)
        n['shape'] = self.eval_node_shape(shape, node)

        return n

    @odoo.http.route('/web_diagram/diagram/get_connector_info', type='json', auth='user')
    def get_connector_info(self, req, id, model, source, destination, label, **kw):
        connector = http.request.env[model].browse(id)

        c = dict()
        c['id'] = connector['id']
        c['name'] = connector['name']
        c['s_id'] = connector[source][0].id
        c['d_id'] = connector[destination][0].id
        c['signal'] = self.eval_connector_label(label, connector)
