...
class XmlWorkflowReader(models.AbstractModel):
    ...
    def read_state(self, element):
        ...
        return {
            ...
            'sequence': self.read_integer(element, 'sequence', default_value=10)
        }
...
class XmlWorkflowWriter(models.AbstractModel):
    ...
    def write(self, workflow, stream, encoding=DEFAULT_ENCODING):
        ...
        :return:
        """
        tree = self._build_xml(workflow, element_tree=True)
        tree.write(stream, encoding=encoding, xml_declaration=True, pretty_print=True)

    def to_string(self, workflow):
        """
        ...
        """
        return etree.tostring(self._build_xml(workflow, element_tree=True), encoding=self.encoding, xml_declaration=True, pretty_print=True)
