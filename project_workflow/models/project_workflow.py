...
# from ..tools.publisher import ProjectWorkflowPublisher
...
class Workflow(models.Model):
        ...
        for workflow in self:
            if len(workflow.project_ids) != 0:
                ...
                raise exceptions.ValidationError(
                    "You are not allowed do delete this workflow because it is being used by the following projects: %s" % projects
                )
    ...
    def get_state_transitions(self, workflow_id, stage_id, task_id):
        ...
        # transitions = [self._populate_state_for_widget(x) for x in workflow.find_transitions(task, stage_id)]
        #
        # global_states = workflow.state_ids.filtered(lambda r: r.is_global == True)
...
class WorkflowState(models.Model):
    ...
    is_default = fields.Boolean(
        string="Is default",
        compute="_get_is_default",
        inverse="_set_default_state",
        help="Stage from this state will be chosen by default if not specified when creating task.",
    )
