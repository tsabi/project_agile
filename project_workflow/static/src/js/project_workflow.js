...
odoo.define('project_workflow.TaskWorkflow', function (require) {
    ...
    var TaskWorkflow = common.AbstractField.extend({
        ...
        render_value: function() {
            ...
            var $content = $(QWeb.render("TaskWorkflowNavigation.content." + ((config.device.size_class <= config.device.SIZES.XS)? 'mobile' : 'desktop'), {
                'widget': self,
            }));
        ...
        get_selection_render: function(render){
            ...
            var calculation = _.bind(function() {
                ...
                return new Model('project.workflow').call('get_state_transitions', [workflow_id, stage_id, self.view.datarecord.id]).then(function (tranitions) {
                    _.each(tranitions, function(transition){
