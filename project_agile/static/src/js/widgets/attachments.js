...
odoo.define('project_agile.attachments', require => {
        ...
        uploadFiles(fileList) {
            if (fileList.length > 0) {
                for (let file of fileList) {
                    ...
                    this.attachmentsList.addItem(newAttachmentItem);
                }
            }
        }
    });
    ...
    const AttachmentsItem = ModelList.AbstractModelListItem.extend({
        ...
        init(parent, options) {
            ...
            if (!this.downloadable) {
                ...
                this.res_model = parent.res_model;
                this.convertTimeout = parent.convertTimeout;
                this._require_obj("file");
                this.record.name = this.file.name;
            }
        },
        willStart() {
        ...
        prepareUserImage() {
            return data.cache.get("get_user", {id: this.record.create_uid[0]}).then(user => {
                this.user_image_url = data.getImage("res.users", user.id, user.__last_update);
            });
        },
        renderElement() {
            this._super();
        ...
        onFileUploaded(id) {
            ...
            setTimeout(this.convertToLink.bind(this), this.convertTimeout);
        },
        onFileUploadError() {
            this.setProgress(0);
            this.$el.addClass("with-error");
    ...
    AttachmentsItem.sort_by = "write_date";
    return {
        AttachmentsWidget,
        AttachmentsItem
    }
