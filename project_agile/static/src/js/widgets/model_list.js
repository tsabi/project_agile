...
odoo.define('project_agile.model_list', function (require) {
    ...
    var ModelList = AgileContainerWidget.extend({
        ...
        getDatasetDefaults(options) {
            ...
            this.items_loaded = this.loadItems();
        },
        loadItems() {
            let def = $.Deferred();
        ...
        willStart() {
            return $.when(this._super(), this.items_loaded);
        },
    ...
    var MessageItem = AbstractModelListItem.extend({
        ...
        image_url() {
            return data.getImage("res.users", this.record.create_uid[0], this.record.author_last_update);
        },
        addedToDOM() {
            this._super();
            this.$('.tooltipped').tooltip({delay: 50});
        ...
        setWriteDate() {
            ...
            let tooltipString = this.record.write_uid[1] + _t(" at ") + this.record.write_date;
