    ...
    const HeaderWidget = AgileBaseWidgets.AgileBaseWidget.extend({
        ...
        renderElement() {
            ...
            data.cache.get("current_user").then(user => {
                ...
                let team = user.team_ids[user.team_id].name;
                ...
                if (Object.keys(user.team_ids).length > 1) {
                    ...
                    for (let team_id in user.team_ids) {
                        ...
                        let teamLi = $("<li><div><img class='circle team-image' src='" + data.getImage("project.agile.team", team_id) + "'/><a data-id='" + team.id + "'>" + team.name + "</a></div></li>");
        ...
        addedToDOM() {
            ...
            this.$('.team-dropdown-button').dropdown({
                ...
                constrain_width: false,
                hover: true,
                gutter: -this.$('#team-dropdown').width(),
                belowOrigin: false,
                alignment: 'left'
            });
