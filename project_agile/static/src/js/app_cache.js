...
odoo.define('project_agile.app_cache', function (require) {
    ...
    data.cache.add("board_data", DependencyCache.AbstractDependency.extend({
        resolve() {
            this._require_obj("params", ["id"]);
            this.cache.get("model_info", {model: "project.agile.board"}).then(model => {
                this.getDataSet(model.name).read_ids([this.params.id], model.fields_list)
                    .then(boards => {
                        this.deferred.resolve(boards[0]);
                    });
            });
    ...
    data.cache.add("team_members", DependencyCache.AbstractDependency.extend({
        resolve() {
            this.getDataSet("project.agile.team").call("all_users_from_my_team").then(this.deferred.resolve, this.deferred.reject);
    ...
    data.cache.add("current_user", DependencyCache.AbstractDependency.extend({
        resolve() {
            data.session.rpc("/agile/session_user").then(user => {
                user.imageUrl = data.getImage("res.users", user.id, user.__last_update);
    ...
    data.cache.add("get_user", DependencyCache.AbstractDependency.extend({
        resolve() {
            this._require_obj("params", ["id"]);
            this.cache.get("team_members").then(members => {
                if (members[this.params.id]) {
                    this.deferred.resolve(members[this.params.id]);
                } else {
