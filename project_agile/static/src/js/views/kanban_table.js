...
odoo.define('project_agile.view.kanban_table', function (require) {
    ...
    class StorySwimlaneDefinition extends AsyncSwimlaneDefinition {
        constructor(kanbanTable) {
            super(kanbanTable, 'story', _("Story"), _t("Other issues"), "project.agile.kanban_table.swimlane.header.story");
        }
    ...
    var TaskCard = AbstractCard.extend({
        ...
        willStart() {
            return this._super().then(() => {
                return $.when(data.cache.get("team_members").then(members => {
        ...
        image_url() {
            return this.user ? data.getImage("res.users", this.user.id, this.user.__last_update) : "/project_agile/static/img/unassigned.png";
        },
        image_tooltip() {
        ...
        _onAssignToMeClick() {
            ...
            data.cache.get("team_members").then(members => {
                this.user = members.find(e => e.id == data.session.uid);
                this.renderElement();
                this.start();
            });
        },
    ...
    var AbstractKanbanTable = BaseWidgets.AgileBaseWidget.extend({
        ...
        renderElement() {
            ...
            this.$(".swimlanes").append(this.renderSwimlanes());
        },
        prepareSwimlaneData() {
        ...
        renderSwimlanes() {
            ...
            if (this.data.swimlanes.size < 1) {
                ...
                this.data.swimlanes.forEach((data, id) => {
                    result.push(this.renderSwimlane(data, id, false));
                });
            } else {
                let sortedSwimlanes = [...this.data.swimlanes.keys()].sort((a, b) => !a ? 1 : -1);
                sortedSwimlanes.forEach(id => {
                    let data = this.data.swimlanes.get(id);
                    result.push(this.renderSwimlane(data, id));
                });
            }
            return result;
        },
        renderSwimlane(data, id, renderHeader = true) {
            let node;
            ...
            for (let column of this.data.sorted_columns) {
                ...
                node.find(".columns").append(this.renderColumn(column, column_cards));
            }
            return node;
        },
        renderColumn(column, records) {
            let swimlaneDefinition = this.swimlanes.get(this.swimlane);
                    ...
                    for (let columnNode of $.makeArray(this.swimlaneNode.find(`.column:not([data-column-id=${columnId}])`))) {
                        ...
                        columnOverlay.find(".state").on("drop", (event, ui) => {
                            ...
                            var changeRecordStage = function (message) {
                                // Set new stageField
                                cardWidget.record[tableThis.stageField] = parseInt(newState.stage_id);
        ...
        openStageChangeModal(newStageId, cardWidget, confirmedCallback) {
            ...
            var modal = new AgileModals.TaskStageConfirmationModal(this, {
                ...
                afterHook: (confirmation, form, result) => {
                    // Update task widget model and rerender
                    confirmedCallback(result);
                    //taskWidget.__model.user_id = {
                    //    id: confirmation.user_id,
                    //    name: form.find("#user_id option:selected").text()
                    //};
                    cardWidget.record[this.stageField] = confirmation.stage_id;
                    cardWidget.renderElement();
                    cardWidget.start();
                }
            });
            modal.appendTo($("body"));
        ...
        addRecord(record) {
            let swimlaneDefinition = this.swimlanes.get(this.swimlane).begin();
            $.when(swimlaneDefinition.mapper(record), swimlaneDefinition.filter(record)).then((group, passIt) => {
                if (passIt) {
                    if (this.data.swimlanes.has(group)) {
                        this.data.swimlanes.get(group).records.push(record);
                        ...
                        return $.when(swimlaneDefinition.header(record)).then(header => {
                            header.recordCount = recordCount.bind(swimlaneData);
                            swimlaneData.header = header;
                            let newSwimlaneNode = this.renderSwimlane(swimlaneData, group);
        ...
        setSwimlane(name, store = true) {
            ...
            this.prepareSwimlaneData().then(() => {
                let newSwimlanes = this.renderSwimlanes();
                this.$(".swimlanes").empty();
                this.$(".swimlanes").append(newSwimlanes);
