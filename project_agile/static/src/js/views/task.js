...
odoo.define('project_agile.view.task', function (require) {
    ...
    var TaskView = BaseWidgets.AgileViewWidget.extend({
        ...
        init(parent, options) {
            ...
            this.task_id = parseInt(hash_service.get("task"));
            if (!this.task_id) {
        ...
        start() {
            ...
            this.$(".assign-to-me").click(() => {
                let taskView = this;
                data.getDataSet("project.task").call('write', [[this.task_id], {'user_id': data.session.uid}]).then(() => {
            ...
            core.bus.on("project.task:write", this, (id, vals, payload) => {
                DataServiceFactory.get("project.task").getRecord(id).then(task => {
                    this.loadTask(id, task);
                })
            });
            core.bus.on("project.task:unlink", this, (id, payload) => {
                // TODO: Remove actions, floating buttons, generate ovelay with message box telling that task has been deleted
            });
            return this._super();
        },
        addedToDOM() {
