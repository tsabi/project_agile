...
odoo.define('project_agile.DataService', function (require) {
    ...
    let DataService = web_core.Class.extend(AgileMixins.RequireMixin, {
        ...
        wrapRecord(record) {
            ...
            return new Proxy(record, {
                set(trapTarget, key, value, receiver) {
                    ...
                    return Reflect.set(trapTarget, key, field.type === "many2one" && !Array.isArray(value) ? [value, undefined] : value, receiver);
                },
                        ...
                        return new Proxy(() => {
                            ...
                            apply: function (trapTarget, thisArg, argumentList) {
                                return new Promise(function (resolve, reject) {
                                    service.dataset._model.call(key, [[record.id], ...argumentList], {context: service.dataset.get_context().eval()})
                                        .then((resolved) => resolve(resolved))
                                        .fail((rejected) => {
                                            reject([rejected, key, argumentList]);
                                        });
                                });
                            }
                        });
        ...
        recordUpdated(id, vals, payload) {
            if (this.records.has(id)) {
                Object.assign(this.records.get(id)._source, vals);
            }
            core.bus.trigger(this.model + ":write", id, vals, payload);
        },
