...
class Project(models.Model):
    ...
    def _get_default_type_id(self, ):
        return xmlid_to_res_id(self, "project_agile.project_type_software")
    ...
    def _set_default_project_type_id(self):
        ...
        project_type = type_id and self.env['project.type'].browse(type_id) or False
        workflow_id = project_type and project_type.workflow_id or False

        for project in self.with_context(active_test=False).search([('type_id', '=', False)]):
            project.type_id = type_id
            if not self.env.context.get("no_workflow", False):
                project.workflow_id = workflow_id

    # TODO: Why is this here? This should be moved to the field definition.
    _defaults = {
    ...
    def open_user_stories(self):
        self.ensure_one()
        action = xmlid_to_action(self, "project.act_project_project_2_project_task_all")
